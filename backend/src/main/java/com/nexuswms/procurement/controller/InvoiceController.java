 ...
 */
@RestController
@RequestMapping("/api/v${spring.mvc.apiversion.supported}/invoices")
...
public class InvoiceController {
    ...
    /**
     * Triggers the matching process for a specific invoice (3-way matching:
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'FINANCE')")
    @ApiResponse(responseCode = "200", description = "Invoice matching process completed successfully")
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'FINANCE')")
    @ApiResponse(responseCode = "200", description = "Invoice retrieved successfully by ID")
